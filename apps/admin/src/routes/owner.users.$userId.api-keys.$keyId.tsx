...
import Page from "../../pages/owner/users/[userId]/api-keys/[keyId]/page";
...
const loadPageData = createServerFn({ method: "GET", strict: false })
  ...
  .handler(async ({ data }) => {
    const { loadPage } = await import("../../pages/owner/users/[userId]/api-keys/[keyId]/page.server");
    return runAdminPageLoader(() => loadPage(data));
  });

export const Route = createFileRoute("/owner/users/$userId/api-keys/$keyId")({
