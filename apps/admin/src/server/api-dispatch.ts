export type AdminRouteContext = { params: Promise<Record<string, string | string[]>> };
export type AdminRouteHandler = (request: Request, context: AdminRouteContext) => Response | Promise<Response>;
export type AdminRouteModule = Partial<Record<string, AdminRouteHandler>>;
...
export async function dispatchApiRoutes(
  request: Request,
  routes: readonly AdminApiRouteDefinition[],
): Promise<Response> {
  const pathname = new URL(request.url).pathname;
  if (!hasValidPathEncoding(pathname)) {
    return Response.json({ error: "invalid_path_encoding" }, { status: 400 });
  }
  for (const route of routes) {
    ...
    const routeModule = route.module as AdminRouteModule;
    const allow = allowedMethods(route.methods);
    if (request.method === "OPTIONS" && !routeModule.OPTIONS) {
      return new Response(null, { status: 204, headers: { allow: allow.join(", ") } });
    }
    const requested = request.method === "HEAD" && !routeModule.HEAD && routeModule.GET
      ? "GET"
      : request.method;
    ...
    try {
      ...
      process.stdout.write(`${JSON.stringify({ event: "admin.head_body.cancel_failed", code: "response_body_cancel_failed" })}\n`);
    }
