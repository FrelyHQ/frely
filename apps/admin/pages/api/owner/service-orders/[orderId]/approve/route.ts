...
export async function POST(request: Request, context: Context) {
  return handle(request, async () => {
    ...
    const input = { orderId, ownerUserId: claims.sub, confirmedReceivedAmountUnits: Number(body.confirmedReceivedAmountUnits), reviewNote: body.reviewNote };
    return json(await application.billingCommands.approveServiceOrder(input));
  });
}
