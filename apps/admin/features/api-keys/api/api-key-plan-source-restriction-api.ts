...
export function createAdminApiKeyPlanSourceRestrictionApi(apiKeyId: string): ApiKeyPlanSourceRestrictionApi {
  return {
    pageCandidates: async ({ query, page, pageSize }, signal) => {
      ...
      const response = await fetch(`/api/owner/api-keys/${encodeURIComponent(apiKeyId)}/plan-source-restriction/candidates?${params.toString()}`, signal ? { signal, cache: "no-store" } : { cache: "no-store" });
      return readConsoleApiResponse<ApiKeyPlanSourceRestrictionCandidatePage>(response, "Failed to load Plan source candidates");
    },
    replace: async (input) => {
      const response = await fetch(`/api/owner/api-keys/${encodeURIComponent(apiKeyId)}/plan-source-restriction`, {
        method: "PATCH",
