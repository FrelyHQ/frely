...
export function WebUiSurfaceProvider({
  children,
  endpoint,
  release,
  routeRegistry,
...
export function WebPageSurfaceContent({
  children,
  ...props
...
function observeWebVitals(registration: DocumentWebVitalsRegistration): void {
  const callback = (metric: MetricType) => {
    if (Number.isFinite(metric.value)) {
      registration.report?.(metric.name as WebVitalName, metric.value, registration.initialRoute);
    }
  };
  onCLS(callback);
  onFCP(callback);
