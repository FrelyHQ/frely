...
import Page from "../../pages/user/plans-and-budgets/budget/page";
...
const loadPageData = createServerFn({ method: "GET", strict: false }).validator(validateWebPageInput).handler(async () => {
  const { loadPage } = await import("../../pages/user/plans-and-budgets/budget/page.server");
  return runWebPageLoader(loadPage);
});

export const Route = createFileRoute("/user/plans-and-budgets/budget")({ loader: () => loadPageData(), component: PageRoute });
